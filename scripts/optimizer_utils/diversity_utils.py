...
class WorkflowSimilarity:
    ...
    def find_similar_workflows(
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if similarity_matrix[i, j] >= similarity_threshold:
                    G.add_edge(candidate_ids[i], candidate_ids[j], weight=similarity_matrix[i, j])

        # 2. 使用 Louvain 算法找到最佳的社区划分
        # partition 是一个字典，格式为 {节点: 社区ID}
        partition = community_louvain.best_partition(G, weight='weight')
        
        if not partition:
            # 如果没有形成任何社区（例如所有节点都是孤立的），则每个节点自成一组
        ...
        all_groups = list(communities.values())
        
        # 将图中未参与社区划分的孤立节点也各自作为一个组
